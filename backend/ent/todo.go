...
func (t *Todo) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case todo.FieldReflectionMemo:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field reflection_memo", values[i])
			} else if value.Valid {
				t.ReflectionMemo = new(string)
				*t.ReflectionMemo = value.String
			}
...
func (t *Todo) String() string {
	...
	builder.WriteString(", ")
	if v := t.ReflectionMemo; v != nil {
		builder.WriteString("reflection_memo=")
		builder.WriteString(*v)
	}
	builder.WriteByte(')')
